...
**Architecture:** A Telegram webhook lands on the same Cloudflare Worker that already serves the site. It stores posts in Cloudflare D1, photos in Cloudflare R2, and optionally runs the submitted text through Cloudflare Workers AI for a spelling/grammar pass before publishing. The site reads posts through a small internal service that queries D1 directly when running on the primary Worker, and falls back to a public JSON API (`GET /api/posts`) when running anywhere else (e.g. the Lovable mirror) — so both surfaces stay live with zero rebuild.

**Tech Stack:** TanStack Start (file-based routes, already in the repo), Cloudflare Workers, D1, R2, Workers AI, Telegram Bot API (plain `fetch`, no SDK), Vitest for unit tests.
...
### Task 1: Cloudflare bindings — D1, R2, Workers AI
...
**Interfaces:**
- Produces: global `Env` interface (from `worker-configuration.d.ts`) with `DB: D1Database`, `PHOTOS: R2Bucket`, `AI: Ai`. All later tasks that touch Cloudflare bindings import this implicitly (it's global, no import needed).
...
Run: `npx wrangler r2 bucket create piraquara-posts-photos`
Expected: `Created bucket 'piraquara-posts-photos'`.
...
```jsonc
{
  ...
  "r2_buckets": [
    {
      "binding": "PHOTOS",
      "bucket_name": "piraquara-posts-photos"
    }
  ],
  "ai": {
    "binding": "AI"
...
Expected: creates/updates `worker-configuration.d.ts` at the repo root with a global `interface Env { DB: D1Database; PHOTOS: R2Bucket; AI: Ai; }`.
...
git commit -m "chore: add D1, R2, and Workers AI bindings"
```
...
### Task 6: Photo storage (R2) + public serving route
...
- Produces: `assertPhotoSizeOk(byteLength)`, `PhotoTooLargeError`, `uploadPhoto(bucket, bytes, contentType): Promise<string>` (returns a relative URL like `/fotos/<uuid>.jpg`), `deletePhoto(bucket, fotoUrl): Promise<void>`. Used by Task 15 (bot executor).
...
export async function uploadPhoto(bucket: R2Bucket, bytes: ArrayBuffer, contentType: string): Promise<string> {
...
export async function deletePhoto(bucket: R2Bucket, fotoUrl: string): Promise<void> {
...
Run: `npx vitest run src/lib/photos-storage.test.ts`
Expected: PASS (2 tests). `uploadPhoto`/`deletePhoto` themselves need a real R2 bucket — covered in Task 20.
...
git commit -m "feat: add photo storage and public /fotos/:key route"
```
